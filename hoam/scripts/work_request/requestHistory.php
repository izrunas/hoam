...
if (isset ($_GET['id'])) {
	...
	if (isset ($_SESSION['current_user']['id']) AND
		...
		$trusted_id = $work_request_class -> superClean ($_GET['id'], $work_request_class -> returnColumnSize ('id'));
		$history_list = $_HOAM_log -> listOwnerBit ($trusted_id, (int) $_HOAM_setting ['log']['flags']['work_request']);
	} else {
		$history_list = array ();
	}
