...
if (isset ($_GET['id'])) {
	...
	if (isset ($_SESSION['current_user']['id']) AND
		...
		$trusted_id = $insurance_class -> superClean ($_GET['id'], $insurance_class -> returnColumnSize ('id'));
		$history_list = $_HOAM_log -> listOwnerBit ($trusted_id, (int) $_HOAM_setting ['log']['flags']['insurance']);
	} else {
		$history_list = array ();
	}
