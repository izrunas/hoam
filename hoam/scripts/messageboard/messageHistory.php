...
if (isset ($_GET['id'])) {
	...
	if (isset ($_SESSION['current_user']['id']) AND
		...
		$trusted_id = $messageboard_class -> superClean ($_GET['id'], $messageboard_class -> returnColumnSize ('id'));
		$history_list = $_HOAM_log -> listOwnerBit ($trusted_id, (int) $_HOAM_setting ['log']['flags']['messageboard']);
	} else {
		$history_list = array ();
	}
