...
if (isset ($_SESSION['current_user']['id']) AND
	...
	if (isset ($_GET['id'])) {
		...
		$trusted_id = $group_class -> superClean ($_GET['id'], $group_class -> returnColumnSize ('id'));
		$group_info = $group_class -> listAll ($trusted_id);
		...
		if (is_array ($group_info)) {
			...
			// Populate the XML file with information about the group.
			xmlwriter_start_element ($xw, 'item');
			
			xmlwriter_start_element ($xw, 'name');
			xmlwriter_write_cdata ($xw, $group_info['name']);
			xmlwriter_end_element ($xw); // name
			
			xmlwriter_start_element ($xw, 'description');
			xmlwriter_write_cdata ($xw, $group_info['description']);
			xmlwriter_end_element ($xw); // description
			
			xmlwriter_write_element ($xw, 'member_count', $group_members_class -> returnCountMembers ($group_info['id']));
