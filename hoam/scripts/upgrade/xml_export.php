...
if ($group_members_class -> isMember ($_HOAM_setting ['group']['ids']['admin'], $_SESSION['current_user']['id']) OR
	...
	// This will need to be expanded in the future to handle backups of all
	// attachments as well, which would be a true backup of all user data.

	if (isset ($_GET['attachments'])) {
		$file = HOAM_upgradeExport (TRUE);
	} else {
		$file = HOAM_upgradeExport (FALSE);
	}
	if ($file) {
		xmlwriter_write_element ($xw, 'size', HOAM_humanFilesize (filesize (HOAM_PATH . 'backup/' . $file)));
		xmlwriter_write_element ($xw, 'filename', $file);
	} else {
		xmlwriter_write_element ($xw, 'size', '');
