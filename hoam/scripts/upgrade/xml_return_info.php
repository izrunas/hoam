...
if ($group_members_class -> isMember ($_HOAM_setting ['group']['ids']['admin'], $_SESSION['current_user']['id']) OR
	...
	if (isset ($_GET['changes']) OR isset ($_GET['details']) OR isset ($_GET['notice'])) {
		...
		if (isset ($_GET['changes'])) {
			xmlwriter_write_cdata ($xw, HOAM_upgradeReturnChanges ());
		} elseif (isset ($_GET['details'])) {
			xmlwriter_write_cdata ($xw, HOAM_upgradeReturnDetails ());
		} else {
			xmlwriter_write_cdata ($xw, HOAM_upgradeReturnNotice ());
		}
