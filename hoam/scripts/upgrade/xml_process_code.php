...
if ($group_members_class -> isMember ($_HOAM_setting ['group']['ids']['admin'], $_SESSION['current_user']['id']) OR
	...
	if (isset ($_GET['PHP']) OR isset ($_GET['SQL'])) {
		...
		if (isset ($_GET['PHP'])) {
			switch ($_GET['PHP']) {
				case 'pre-check' :
					$success = HOAM_upgradeEval ($upgrade_script ['pre-check']['PHP']);
					break;
				case 'migration' :
					$success = HOAM_upgradeEval ($upgrade_script ['migration']['PHP']);
					break;
				case 'post-check' :
					$success = HOAM_upgradeEval ($upgrade_script ['post-check']['PHP']);
					break;
				case 'cleanup' :
					$success = HOAM_upgradeEval ($upgrade_script ['cleanup']['PHP']);
					break;
				default:
			...
			switch ($_GET['SQL']) { 
				case 'pre-check' :
					$success = HOAM_upgradeSQL ($upgrade_script ['pre-check']['SQL']);
					break;
				case 'migration' :
					$success = HOAM_upgradeSQL ($upgrade_script ['migration']['SQL']);
					break;
				case 'post-check' :
					$success = HOAM_upgradeSQL ($upgrade_script ['post-check']['SQL']);
					break;
				case 'cleanup' :
					$success = HOAM_upgradeSQL ($upgrade_script ['cleanup']['SQL']);
					break;
				default:
					header ('Location: /error/unknown_action/');
					exit ();
