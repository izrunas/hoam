...
if (isset ($_GET['id'])) {
	...
	if (isset ($_SESSION['current_user']['id']) AND
		...
		$trusted_id = $property_class -> superClean ($_GET['id'], $property_class -> returnColumnSize ('id'));
		$history_list = $_HOAM_log -> listOwnerBit ($trusted_id, (int) $_HOAM_setting ['log']['flags']['property']);
	} else {
		$history_list = array ();
	}
